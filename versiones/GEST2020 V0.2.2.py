	...
	def select_theme(self):	
		...
		for index in manager.theme_names:
			Radiobutton(self.window, 
				...
				command=self.change_theme).grid(sticky=W+E, columnspan=1, pady=5, padx=20)

	def change_theme(self):
		self.win_to_mod.config(theme=manager.theme_names[self.theme_sel.get()])
		manager.configs[self.configs.get('theme_name')] = manager.theme_names[self.theme_sel.get()]
		manager.save_config()
		self.window.destroy()
...
class Database:
	...
	def show_data(self, *args, like='%', col_search=0, col_order=0, limit=''):
		...
		print('!!!!!!!!!'+self.entry_array[col_search].get(), col_search)
	...
	def add_record(self, *args):
		...
		self.show_data(like=fail_add)  # like = %: busca la db con "codigo%"
...
class Maestro(Database):
	...
	def __init__(self, window, table_name='MAESTRO'):
		...
		print('esto maestro:'+self.tree.item(self.tree.selection())['text'])
		window.bind('<Control-c>', Listas)  # copia lista con nuevo codigo
		window.bind('<Control-C>', Listas)  # copia lista con nuevo codigo
		window.bind('<Control-f>', Listas)  # Elimina lista seleccionada
		window.bind('<Control-F>', Listas)  # Elimina lista seleccionada
...
class Ordenes_trabajo(Database):
	def __init__(self, *args, table_name='OT'):
		ot_window = Toplevel()
		WindowConfig(ot_window)

		super().__init__(ot_window, table_name)

		# Deshabilita entrys precios (son automatizados)
...
if __name__ == '__main__':
	...
	print(type(root))
	# Setea ventana al 80% de la pantalla
	monitor_x = str(int(GetSystemMetrics(0)*0.8))
	monitor_y = str(int(GetSystemMetrics(1)*0.8))
	root.geometry(f'{monitor_x}x{monitor_y}+100+5')  # "width_x_height_+off_x_+off_y"

	main_window = WindowConfig(root, title='GEST2020 | Sistema de Administración de Producción L.I.E. S.R.L.')

	# edición del menú
	#main_window.create_menu.edit_menu().add_command(label="Copiar Lista (ctrl+C)", command=lambda: Listas('keysym=c'))
	#main_window.create_menu.edit_menu.add_command(label="Eliminar Lista (ctrl+F)", command=lambda: Listas('keysym=f'))
