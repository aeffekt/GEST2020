...
class WindowTheme:
	...
	def __init__(self, win_to_mod, *args):
		if WindowTheme.window == None:
			...
			WindowTheme.window.iconbitmap('_LOGOLIE x3.ico')
	...
	@staticmethod
	def exit_handler():
		WindowTheme.window.destroy()
		WindowTheme.window = None
...
class CreateDatabase:
	...
	def create_triggers():
		...
		query = '''CREATE TRIGGER ot_monto_update
             ...
             BEGIN
                 UPDATE "OT" SET "MONTO" = ROUND(new.PRECIO * new.CANT, 3) WHERE "PLAN" = new.PLAN AND "CÓDIGO" = new.CÓDIGO;
             END
             ;
             '''
		CreateDatabase.conn.execute(query)
	...
	def delete_null(xls_file, column):
		for item in range(len(xls_file[column])):
			if xls_file[column][item] == 'nan':
				print('es none: ', xls_file[column][item])
				xls_file[column][item] = 'EX-NULL'
				print('no es none: ', xls_file[column][item])
		return xls_file
...
class ManageTable:
	...
	def __repr__(self):
		return f'ManageTable("{self.window}", "{self.table_name}")'
	...
	def valid_add(self):
		...
		try:
			for row in db_rows:
				if str(row).find(self.entry_array[0].get()) != -1:
					return False
			return True
		except:
			return True
			...
			for id in selection:
				...
				for index, column in enumerate(self.table_columns):
					...
					if index not in self.no_edit_entry and self.entry_array[index].get() != '' and str(self.entry_array[index].get()) != 'None':
						#para editar un item, lo comprueba el "COD y descr" o "lista y COD" o "PLAN y COD"
						if index == 0:
...
class Listas(ManageTable):
	...
	def query_search(self, *args, search='', like='%', col_search=0, col_order=1, order='DESC', limit=''):
		if self.table_columns.size > 0:
			...
			like_line = f' LIKE "{search}{like}"'
			order_line = f' ORDER BY "{self.table_columns[0]}" {order}, "{self.table_columns[col_search]}" {order}'
		else:
	...
	def valid_add(self):
		...
		if self.entry_array[0].get() == '':
			return False
		# checkea que el item no exista ya
...
class OrdenTrabajo(ManageTable):
	...
	def __init__(self, *args, table_name='OT'):
		...
		if OrdenTrabajo.window == None:
			...
			OrdenTrabajo.window.bind('<Control-X>', lambda b: OrdenCompra(buy_order=self.entry_array[0].get()))
			OrdenTrabajo.window.bind('<Control-x>', lambda b: OrdenCompra(buy_order=self.entry_array[0].get()))
			self.show_data(open=False)
	...
	# valida el agregar un item
	def valid_add(self):
		# el item a agregar no debe estar vacio
		if self.entry_array[0].get() == '':
			return False
		# checkea que el item no exista ya
		if self.entry_array[1].get() == '':
		...
		query = f'SELECT OT.{self.table_columns[0]}, OT.{self.table_columns[1]} ' \
				f'FROM "OT" WHERE "PLAN" = "{self.entry_array[0].get()}" AND CÓDIGO = "{codigo}"'
		db_rows = self.run_query(query)
		try:
			for row in db_rows:
				if str(row).find(self.entry_array[0].get()) != -1:
	...
	def exit_handler(self):
		OrdenTrabajo.window.destroy()
		OrdenTrabajo.window = None
...
def help_info():
	...
	help_tree.insert(help_two_a, tk.END, text='Primero debe crear la lista en el maestro de artículos: Debe ser con Nombre único')
	help_tree.insert(help_two_a, tk.END, text='Abrir haciendo doble click sobre el nuevo código. Por último agrega los elementos que desea.')
