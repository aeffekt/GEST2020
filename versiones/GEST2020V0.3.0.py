...
import os                       # impresión de shell "print"
import itertools 				# herraminetas de iteracion (count)
import functools				# se usa para sort column del tree
import inspect					# análisis de objetos
import logging 					# log to file
import decimal 					# se usa para determinar cantidad de decimales de algunos numeros
...
import time 					# para uso de timer (decorador que mide el tiempo de ejecucion)
...
window_icon = 'LOGOLIE x3.ico' # icono de cada ventana
backup_max = 1	# cantidad de archivos backup

# instancia de logger maestro, asi permite autonimia de configuracion
logger = logging.getLogger(__name__) 
logger.setLevel(logging.WARNING)	# nivel de logger [INFO:DEBUG:WARNING:ERROR:CRITICAL]
...
logger.addHandler(file_handler)	# asocia para escribir a archivo
logger.addHandler(stream_handler)	# asocia para mostrar en consola


#-----------------------------CLASES--------------------------------#

# Configuraciones, usuarios (a futuro), cierre final, etc
class ProgManager:
	...
	def file_backup(file_path):
		if backup_max > 0:
			with open(file_path, 'rb') as file:
				path_backup = ProgManager.backup_name(file_path)
	...
	@staticmethod
	def backup_name(file_path, num=0, fechas_mod={}):
		path, ext = file_path.split('.')	
		path_backup = path+'_backup_'+str(num)+'.'+ext
		if os.path.isfile(path_backup) and num < backup_max:
			fechas_mod[os.path.getmtime(path_backup)] = num
			return ProgManager.backup_name(file_path, num+1, fechas_mod)
			...
			with open(self.configs['path_config'], 'r') as config_file:
				...
				for index, key in enumerate(self.configs):
					try:
						...
						messagebox.showwarning('LEER', 'Ocurrió un error al cargar las configuraciones iniciales.')
...
class WindowConfig:
	...
	def __init__(self, parent, *args, **kwargs):
		...
		self.window.iconbitmap(window_icon)  # icono e la ventana
	...
	def create_menu(self):
		...
		# Menu ayuda
		self.help_menu.add_command(label="Ayuda (CTRL+H)", command=WindowConfig.help_info)
		self.help_menu.add_command(label="Licencia", command=WindowConfig.license)
		self.help_menu.add_command(label="Comandos rápidos", command=WindowConfig.hotkeys)
		self.help_menu.add_separator()
		self.help_menu.add_command(label="Acerca de GEST2020", command=WindowConfig.help_about)
	...
	def help_info(*args):
		...
		help_tree.insert(help_two_a, tk.END, text='y puede ir agregando los elementos desde maestro que desea pertenezcan a dicha lista.')
		...
		help_tree.insert(help_two, tk.END, text='Abra la Lista o ORDEN a editar y complete los campos de edición para agregar un ítem (CTRL+A)')
		help_tree.insert(help_two, tk.END, text='Sino conoce bien el código, deje el campo CÓDIGO vacío, y vaya al Maestro de articulos.')
		help_tree.insert(help_two, tk.END, text='lo selecciona desde el maestro y vuelve a la ventana de Lista u Órden de destino. ')
	...
	def help_about():  # funcion para vent emergente que muestra info con icono de info
		messagebox.showinfo('Gestor de artículos', 'GEST2020 Versión: V0.3.0\n\nProgramado por Agustin Arnaiz'
...
class WindowTheme:
	...
	def __init__(self, win_to_mod, *args):
		if WindowTheme.window == None:
			...
			WindowTheme.window.iconbitmap(window_icon)
...
class CreateDatabase:
	...
	def format_table(name):
		...
		if name == 'MAESTRO':
			...
			xls_file = CreateDatabase.delete_decimal(xls_file, 'PRECIO1,N,10,3')
			xls_file = CreateDatabase.delete_null(xls_file, 'RUBRO,C,10')
			# creacion de tabla
		...
		if name == "LISTAS":
			...
			xls_file = CreateDatabase.delete_decimal(xls_file, 'CANT,N,10,3')
		...
		if name == "OT":
			...
			xls_file = CreateDatabase.delete_decimal(xls_file, 'PRUNIT,N,10,3')
			xls_file = CreateDatabase.delete_decimal(xls_file, 'MONTO,N,10,3')
		...
		CreateDatabase.copy_xls_db(xls_file, name, query)

	# Elimina valores NULL de la columna
	@staticmethod
	def delete_null(xls_file, column):
		for item in range(len(xls_file[column])):
			if str(xls_file[column][item]) == 'nan':
				xls_file[column][item] = 'NONE'
		return xls_file

	# Elimina decimales no significativos
	@staticmethod
	def delete_decimal(xls_file, column):
		for index, item in enumerate(xls_file[column]): 
			if abs(decimal.Decimal(str(item)).as_tuple().exponent) > 3:
				xls_file[column][index] = round(item, 3)
		return xls_file

	# crea la tabla dentro de la base de datos
	@staticmethod
	def copy_xls_db(xls_file, name, query):
		print('copiando ', name, '...')
		try:
			cursor = CreateDatabase.conn.cursor()
			cursor.execute(query)
			#copia los datos dentro de esa misma tabla
			xls_file.to_sql(name=name, con=CreateDatabase.conn, if_exists='append', index=False) #if_exist='replace' resulta en falla PK FK etc
		except Exception as err:
...
class ManageTable:
	...
	def __init__(self, window, table_name):
		...
		#sin implementar aún
		self.progress_bar = ttk.Progressbar(orient="horizontal", length=100, mode="determinate").grid(row=10, column=10)
	...
	def build_main_view(self):
		...
		for index, column in enumerate(self.table_columns):
			...
			sort_f = functools.partial(self.treeview_sort_column, column, False)
	...
	def run_query(self, query, parameters={}, many=False):
		with sqlite3.connect(manager.configs.get('db_name')) as conn:
			try:
				...
				self.window.lift()
				logger.exception(f'error con:{err}-{query}-{parameters}')
			else:
				conn.commit()
				return result
	...
	def show_data(self, *args, like='%', col_search=0, col_order=0, limit='', open=True):	
		...
		cursor = self.query_search(*args, search=self.entry_array[col_search].get(), like=like, 
									col_search=col_search, col_order=col_order, limit=limit)
	...
	def valid_add(self):
		# ni el item a agregar o el codigo de lista/orden debe estar vacio
		try:
	...
	def add_query(self):
		...
		for index, column in enumerate(self.table_columns):
			...
			if index not in self.no_edit_entry:
				# y solo agrega si tiene contenido el entry
	...
	def edit_record(self, *args):
		try:
			# Si hay un solo item en pantalla, lo selecciona para edicion
			if len(self.tree.get_children()) == 1:
				self.tree.selection_set(self.tree.get_children())
			selection = self.tree.selection()
			# solo edita si hay algo para editar, y no tiene codigo = ''
			if len(self.tree.selection()) == 0:
				raise Exception('Debe seleccionar un item para editarlo.')
			else:
				if self.entry_array[0].get() == '' and len(selection) == 1:
					raise Exception('No se puede ingresar un código vacío')			
				elif len(selection) > 1:
					# para multiple edicion, checkea la columna que se modificó
					edit_column = []
					for index, column in enumerate(self.table_columns[1:], start=1):
						if self.entry_array[index].get() != self.tree.item(selection[-1])['values'][index-1]:
							edit_column.append(column)
		except Exception as err:
			messagebox.showwarning('ADVERTENCIA', f'{err}!')
			self.window.lift()
		else:
			...
			for id in selection:
				...
				for index, column in enumerate(self.table_columns):
					...
					if index not in self.no_edit_entry and self.entry_array[index].get() != '' and str(self.entry_array[index].get()) != 'None':
						#para editar un item, lo comprueba el "COD y descr" o "lista y COD" o "ORDEN y COD"
						if index == 0:
						...
						# si hay mas de una, para el codigo toma el original
							...
							# para los demas items, toma los entrys (con posible edicion)
								...
								if str(edit_column).find(column) != -1:
									parameters.insert(index, self.entry_array[index].get())
								else:
									parameters.insert(index, values[index-1])
	...
	def delete_record(self, *args):
		try:
			...
			messagebox.showwarning('Atención','{err}')
			self.window.lift()
		else:
			for id in selection:
				...
				self.run_query(query, parameters)
				self.message['text'] = f'El registro {cod_selected} {descr_selected} ha sido eliminado'
			if len(selection) > 1:
	...
	def load_edit_item(self, *args):
		...
		for index, entry in enumerate(self.entry_array):
			if index == 0:
				...
				# En algunas vistas no hay values, por eso el try
				try:
	...
	def treeview_sort_column(self, col, reverse=False):
		try:
			table_col_id = [(self.tree.set(id, col), id) for id in self.tree.get_children('')]
			table_col_id.sort(reverse=reverse)
			for index, (_, id) in enumerate(table_col_id):
...
class Listas(ManageTable):
	...
	def query_search(self, *args, search='', like='%', col_search=0, col_order=1, order='DESC', limit=''):
		...
		query = f'SELECT {self.table_name}.*' \
				f', {maestro.table_name}.{maestro.table_columns[1]}, {maestro.table_name}.{maestro.table_columns[3]}, {maestro.table_name}.{maestro.table_columns[6]} ' \
	...
	# titulo a mostrar despues de cada busqueda, define datos de la ultima busqueda
	def text_frame(self, open=False, col_search=0):
		if open and col_search == 0:
			result = maestro.query_search(search=self.entry_array[col_search].get(), limit='limit 1')
			data = np.array(result.fetchone())
			return str(data[[0,1,6]])
		elif open:
			return str(self.entry_array[col_search].get())
	...
	def add_query(self, *args):
		# Carga los entrys los datos desde maestro si no hay codigo en listas
		...
		if self.entry_array[1].get() == '':
			...
			self.entry_array[2].insert(END, item_found[3]) # precio
	...
	def copy_list(self, *args):
		...
		if maestro.add_record():
			...
			cursor = self.query_search(search=maestro.tree.item(maestro.tree.selection())['text'], like='')
			arg_query = ['?' for column in range(len(self.table_columns)-len(self.no_edit_entry))]		
			parameters = [(maestro.entry_array[0].get(), row[1], row[2], row[3]) for row in cursor]
	...
	def build_print(self):
		...
		valorizada = messagebox.askquestion('Impresión','¿Desea imprimir la lista valorizada?')
		self.window.lift()
		for id in self.tree.get_children():
			item = self.tree.item(id)['text']
			descr_item = self.tree.item(id)['values']
			if valorizada == 'no':
				descr_item[4] = 0
			precio = round(float(descr_item[1]) * float(descr_item[4]), 3)
			descr_item.insert(0, item)
			precio_lista += precio
			if descr_item != '':
		...
		for row in list_row:
			...
			row[1] = row[3]
			row[3] = temp
		result = maestro.query_search(search=self.entry_array[0].get(), limit='limit 1')
		descripción = np.array(result.fetchone())
		list_row.insert(0, ('', ''))
		list_row.insert(0, (f'Costo Total:  u$s {round(precio_lista, 3)}', ''))
		list_row.insert(0, (f'Fecha de Impresión: {date.today()}', ''))
		list_row.insert(0, (f'Fecha de alta: {descripción[5]}', ''))
		list_row.insert(0, (f'Descripción: {descripción[1]}', ''))
		list_row.insert(0, (f'Código Lista: {descripción[0]}', ''))
		list_row.insert(0, ('', ''))
		list_row.insert(0, ('L.I.E. S.R.L. - Sistema de Administración de Producción GEST2020', ''))
		return list_row
...
class OrdenTrabajo(ManageTable):
	...
	def orden_compra(self, *args):
		try:
			...
			messagebox.showwarning('AVISO', f'{err}')
			self.window.lift()
		else:
			for id in self.tree.get_children():
				cod_lista = self.tree.item(id)['values'][0]
				cursor = self.search_lista(cod_lista)
				lista = cursor.fetchall()
				for item in lista:
					item = list(item)
					# multiplica la cantidad del elemento por la cantidad de la lista
					item[1] = round(float(item[1])*float(self.tree.item(id)['values'][1]), 3)
					# calcula el monto en base a la cantidad y precio
					item.insert(3, round(item[1]*item[2], 3))
					# si el código ya existe en la compra lo identifica en el tree, y solo suma a la cantidad
					found = False
			...
			messagebox.showinfo('AVISO', 'Órden de compra finalizada.')
			self.window.lift()
	...
	def build_print(self):
		...
		costo_ORDEN = 0
		for id in self.tree.get_children():
			costo_ORDEN += round(float(self.tree.item(id)['values'][3]), 3)
			list_row.append(self.tree.item(id)['values'])
		list_row.insert(0, (self.table_columns[1:])) #deja de lado el titulo de la columna de ORDEN
		#intercambia orden columnas de unidad por descripción
		for row in list_row:
			temp = row[1]
			row[1] = row[3]
			row[3] = temp
		list_row.insert(0, ('', ''))
		list_row.insert(0, (f'Costo Total:  u$s {round(costo_ORDEN, 3)}', ''))
		list_row.insert(0, (f'Fecha de Impresión: {date.today()}', ''))
		list_row.insert(0, (f'Órden de Compra: {self.entry_array[0].get()}', ''))
		list_row.insert(0, ('', ''))
		list_row.insert(0, ('L.I.E. S.R.L. - Sistema de Administración de Producción GEST2020', ''))
		return list_row
...
class ToPrinter:
	...
	def final_print(self):
		self.to_print.to_excel(self.file_print, 'Hoja1', index=False, header=False)  # index quita columna de indice, header nom columnas
...
if __name__ == '__main__':
	...
	if os.path.isfile(manager.configs['db_name']):
		...
		#except Exception as err:
		#	messagebox.showwarning('Advertencia', f'Hubo un problema al querer mostrar la base de datos:\n{err}')
	else:
