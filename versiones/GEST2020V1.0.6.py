...
class ManageTable:
	...
	def edit_record(self, *args):
		# condiciones básicas para editar, si hay uno solo lo selecciona, que no esté vacío etc
		try:
			...
			if len(selection) == 0:
				...
				if self.entry_array[0].get() == '' and len(selection) == 1:
					...
					# AGREGAR CONSULTA POR EDICION GRANDE QUE PERMITA CANCELAR 
						...
						# si hay mas de una, para el codigo toma el original
							...
							# para los demas items, toma los entrys (con posible edicion)
								...
								if str(edit_column).find(column) != -1:
									...
									print(f"columna a editar: {column}" )
								else:
									parameters.insert(index, values[index-1])

									print(f"columna NO editar: {column}" )
...
class Listas(ManageTable):
	...
	def build_print(self, df, heading_list):
		...
		boton_aceptar = ttk.Button(frame_print, text="Aceptar", command=lambda: window_print.destroy())
