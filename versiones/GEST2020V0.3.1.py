...
from tabulate import tabulate	# justify left dataframe
...
from UpperEntry import UpperEntry
...
class WindowConfig:
	...
	def help_about():  # funcion para vent emergente que muestra info con icono de info
		path = __file__
		version = path.find('V', 0, -1)
		messagebox.showinfo('Gestor de artículos', f'GEST2020 Versión: {path[version:-3]}\n\nProgramado por Agustin Arnaiz'
...
class ManageTable:
	...
	def treeview_sort_column(self, col, reverse=False):
		...
		if col == '#0':
			tree_col_id = [(self.tree.item(iid)["text"], iid) for iid in self.tree.get_children()]
		else:
...
class Listas(ManageTable):
	...
	# override de listas, agrega valorizada y reordena
	def build_print(self, df, heading_list):
		cost_lista = 0
		...
		# lista común
			...
			heading_list.insert(2, f'Lista: {self.entry_array[0].get()}')
			from_obj = self.table_name+'-'+self.entry_array[0].get()
		...
		valorizada = messagebox.askquestion('Impresión','¿Desea imprimir la lista valorizada?', parent=self.window)
		if valorizada == 'yes':	
...
class OrdenTrabajo(ManageTable):
	...
	def orden_compra(self, *args):
		try:
			...
			for index, id in enumerate(self.tree.get_children(), start=1):
				self.progress_bar['value'] = step * index
				self.window.update()
	...
	def inst_master():
		if os.path.isfile(manager.configs['db_name']):
			try:
				...
				messagebox.showwarning('Advertencia', f'Hubo un problema al querer abrir la base de datos:\n{err}')
		else:
