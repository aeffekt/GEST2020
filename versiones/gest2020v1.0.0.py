...
import os                       # check path + impresión de shell "print" *(not in use)
...
#-----------------------------CLASES--------------------------------#
# Manejo de aspectos generales del programa: Logger, Configuraciones, @timer, cierre final
class ProgManager:
	
	def __init__(self, *args):
		# instancia de logger maestro, asi permite autonomía de configuracion
		self.logger = logging.getLogger(__name__) 
		self.logger.setLevel(logging.DEBUG)	# nivel de logger [INFO:DEBUG:WARNING:ERROR:CRITICAL]
			...
			with open(self.configs['path_config'], 'r') as config_file:
				...
				for index, key in enumerate(self.configs):
					try:
						...
						messagebox.showwarning('LEER', 'Ocurrió un error al cargar las configuraciones iniciales.', parent=root)
...
class WindowConfig:
	...
	def help_about():  # funcion para vent emergente que muestra info con icono de info
		messagebox.showinfo('Gestor de artículos', f'GEST2020 Versión: {version}\n\nProgramado por Agustin Arnaiz')
...
class ManageTable:
	...
	def edit_record(self, *args):
		try:
			...
			if len(self.tree.get_children()) == 1 and len(selection) == 0:
