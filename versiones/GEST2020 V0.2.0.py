...
class CreateDatabase:
	def __init__(self):
		self.db_name = sqlite3.connect('db_gest2020_v2.0.db')
		try:
			self.maestro = self.open_xls('MAESTRO')
			self.listas = self.open_xls('LISTAS')
			self.ot = self.open_xls('OT')
			self.db_name.commit()
			self.db_name.close()
	...
	def open_xls(self, name):
		...
		if name == 'MAESTRO':
			query = 'CREATE TABLE "MAESTRO" ' \
					'("index" INTEGER,' \
					'"COD,C,15" TEXT NOT NULL, ' \
					'"DES,C,40" TEXT, ' \
					'"UM1,C,2"	TEXT, ' \
					'"PRECIO1,N,10,3"	REAL, ' \
					'"FEPRECIO,D"	TIMESTAMP, ' \
					'"FEALTA,D"	TIMESTAMP, ' \
					'"LISTA,L"	INTEGER, ' \
					'"STKMIN,N,12,3"	REAL, ' \
					'"RUBRO,C,10"	TEXT, ' \
					'"COMPRASUG,N,10,3"	REAL,' \
					'PRIMARY KEY("COD,C,15"))'
		...
		if name == "LISTAS":
			query = 'CREATE TABLE "LISTAS" (' \
					...
					'"CANT,N,10,3"	REAL,' \
					'"UM1,C,2"	TEXT,' \
					'FOREIGN KEY("COD,C,15") REFERENCES "MAESTRO"("COD,C,15") ON UPDATE CASCADE ON DELETE CASCADE,' \
					'FOREIGN KEY("COMPO,C,15") REFERENCES "MAESTRO"("COD,C,15") ON UPDATE CASCADE ON DELETE CASCADE,' \
					'PRIMARY KEY("index" AUTOINCREMENT))'

		if name == "OT":
			query = 'CREATE TABLE "OT" (' \
					'"index"	INTEGER NOT NULL,' \
					'"COMPRO,N,8,0"	INTEGER,' \
					'"COD,C,15"	TEXT NOT NULL,' \
					'"CANT,N,4,0"	INTEGER,' \
					'"FEVEN,D"	REAL,' \
					'"FECUM,D"	REAL,' \
	...
	def copy_xls_db(self, xls_file, name, query):
		...
		try:
			...
			xls_file.to_sql(name=name, con=self.db_name, if_exists='append')#, index=False
		except:
...
class Database:
	def __init__(self, window, table_name):
		...
		self.db_name = 'db_gest2020_v2.0.db'
		self.sheet_name = table_name
		self.entry_array = []
	...
	def build_main_view(self):
		...
		for each_column in self.sheet_columns[1:]:
			...
			self.tree.column("#" + str(num_column), width=60+(len(largest)*5), minwidth=30, stretch=False)
...
class Listas(Maestro):
	def __init__(self, *args, table_name='LISTAS'):
		...
		# solo abre lista seleccionada en maestro
		try:
			print('en maestro desde listas')
			maestro.tree.item(maestro.tree.selection())['text'][0]
		except:
			listas_window.destroy()
			return
	...
	def cursor_to_dataframe(self, query):
		...
		for list_row in cursor_list:
			...
			# Se da cuando el item no existe en maestro, no deberia pasar
			if maestro_row is None:
				maestro_row = ('NO SE ENCUENTRA EL REGISTRO', 0)
	...
	def build_main_view(self):
		...
		for each_column in nombres_columnas:
			...
			self.entry_array.append(self.entrys(frame=self.frame_tree,
												name=each_column, row=7, column=num_column, width=len(largest) + 4))
			num_column += 1
		#carga en lista para buscar el codigo de lista de maestro
	...
	def print_list(self, *args):
		...
		all_table.insert(0, ('', '', 'Descripción: ', '', '', f'{maestro.entry_array[1].get()}'))
		...
		for index in range(2, len(all_table[-1])): #recorre las columnas desde dos hasta la ultima
			max_len = 2  # tamaño mínimo
			for row in all_table[10:]:   #deja fuera el encabezado del archivo y recorre las filas
				max_len = max(max_len, len(str(row[index])))
			worksheet.set_column(index-2, index-2, max_len+2)   #corrige el index del xlsx con el index de columna
	#try:
		file_print.save()
...
class Ordenes_trabajo(Listas):
	def __init__(self, table_name='OT'):
...
def help_info():
	...
	help_tree.insert(help_one, tk.END, text='Luego debe ir a menú "archivo/Crear base de datos".')
...
if __name__ == '__main__':
	...
	# menu de ARCHIVO
	file_menu = tkinter.Menu(bar_menu, tearoff=0)
	file_menu.add_command(label="Crear base de datos GEST2020", command=CreateDatabase)
	file_menu.add_separator()
	...
	try:
		...
		messagebox.showwarning('Advertencia', 'No se encuentra la base de datos"\n'
					'vaya a menú "archivo/crear base de datos" para generarla.\n'
