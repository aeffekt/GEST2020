...
class Database:
    def __init__(self, window, db_name, sheet_name='hoja1'):
        ...
        self.window.bind('<Control-c>', Listas.copy_list) #copia una lista de maestro
    ...
    def database_to_tree(self, *args, like=True, column=0):   #la columna 0 suele ser index, el código es la #1
        self.delete_tree()
        valores=[]
        if like:
            query = f'SELECT * from "{self.sheet_name}" WHERE "{self.sheet_columns[column + 1]}" ' \
                f'LIKE "{self.entrys_array[column].get()}%" ORDER BY "{self.sheet_columns[column + 1]}" DESC'
        else:
            query = f'SELECT * from "{self.sheet_name}" WHERE "{self.sheet_columns[column + 1]}" ' \
                    f'LIKE "{self.entrys_array[column].get()}" ORDER BY "{self.sheet_columns[column + 1]}" DESC'
        dataframe = self.run_query(query)
...
class Listas(Database):
    def __init__(self, db_name='LISTAS.db'):
        ...
        listas_window.bind('<Control-p>', self.print_data)
    ...
    def print_data(self, *args):
        ...
        query = f'SELECT * from "{self.sheet_name}" WHERE "{self.sheet_columns[1]}" ' \
                f'LIKE "{self.entrys_array[0].get()}" ORDER BY "{self.sheet_columns[1]}" DESC'
        dataframe = self.run_query(query)
        all_table = dataframe.fetchall()
        df = pd.DataFrame(all_table) #crea el DATAFRAME
        df.to_excel(file_print, 'Hoja1', index=False) #index false es pàra que no agregue una columna de indice
        try:
            file_print.save()
    ...
    def copy_list(self, *args):
        try:
            lista.entrys_array[0].get()  # viejo código de lista
            if maestro.add_record():    #copia los datos de la vieja lista, con nuevo nombre en maestro
                query = f'SELECT "{lista.entrys_array[0]}" FROM "{lista.sheet_name}"'
                dataframe = lista.run_query(query)
                rows = dataframe.fetchall()
                for row in rows:
                    pass
                maestro.entrys_array[0].get()  # nuevo codigo de lista
            else:
                maestro.message['text'] = "No se copió ningún registro"
                return
        except:
            messagebox.showwarning('Adevertencia', 'Debe hacer doble click sobre un registro, para copiar la lista')
...
if __name__ == '__main__':
    ...
    edit_menu.add_command(label="Copiar Lista (ctrl+c)", command=lambda: maestro.copy_list())
